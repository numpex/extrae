...
#define MEMUSAGE_BASE 550

extrae_plugin_ev memusage_events[] = 
{
...
int memusageGetEvent(const char *name_in, extrae_plugin_ev *event_out, extrae_type_t *type_out)
{
    for(int i=0; i<total_num_events; i++)
    {
        if( strcmp(memusage_events[i].name, name_in) == 0 )
        {
            event_out->name = memusage_events[i].name;
            event_out->description = memusage_events[i].description;
            *type_out = MEMUSAGE_BASE+i;
...
{
  printf("hola  desds me  usage \n");
  char *params;
  char *name;

  total_num_events = sizeof(memusage_events) / sizeof(memusage_events[0]);
  active_events = malloc(total_num_events * sizeof(extrae_plugin_ev));  
  types = malloc(total_num_events * sizeof(extrae_type_t));

  //read envar
  params = getenv ("EXTRAE_PLUGIN_MEMUSAGE_PARAM");
  if ( params ==   NULL )
  {
  ...
  while (name != NULL)
  {
      int found = memusageGetEvent(name, &active_events[numevents], &types[numevents]);
      if( found )
      {
      numevents++;
      }
